...
# result_table contiene la información de resultados de ARPA más metadatos contenidos 
# en el objeto metadata.
# IMPORTANTE: result_table solo puede contener la info de una placa que corresponda con
#            los resultados de qc de la placa.
...
plateBooklet <- function(result_table, outdir, qc_input){
    ...
    # Lógica de calidad de la placa. basada en qPCR_pipeline_COVID19/src/functions.R linea 339
...
makeReports <- function(result_table, # data with columns like simulated data
                         outdir, # The report output directory will be defined either by the user or by the DDT
                         qc_input, # The qc output from ARPA
                         report = FALSE){
      if(report == FALSE){ # reporte para el cliente
        lapply(split(result_table,result_table$id_client), resultsReport, outdir = outdir)
      }else{ # reporte para el lab
