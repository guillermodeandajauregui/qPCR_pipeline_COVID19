...
\huge \bfseries{\scshape{Reporte de calida de la placa qPCR}}\\
...
Placa: $if(plate)$\texttt{$plate$}$else$TBD$endif$ & Fecha del reporte: $if(date)$\texttt{$date$}$else$TBD$endif$\\
 Número de controles: $if(ncontrols)$\texttt{$ncontrols$}$else$TBD$endif$ & Resultado: $if(controls)$\texttt{PASS}$else$\cellcolor{red!50}\texttt{FAIL}$endif$\\
	\hline
	\end{tabular*}
}	
\vspace{1mm}
...
\begin{center} 
\normalsize \bfseries{\slshape{$plate$ amplifaction plots}}
\end{center}
