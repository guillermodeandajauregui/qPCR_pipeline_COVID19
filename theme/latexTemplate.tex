...
% \SetWatermarkText{\begin{minipage}{30cm}\centering \vskip 100pt \includegraphics{theme/watermark.pdf} \end{minipage}}
% \SetWatermarkAngle{0}
...
$if(report)$
\begin{center} 
\Large \bfseries{\scshape{qPCR COVID-19}}\\
\normalsize \bfseries{\scshape{Report}}
\end{center}
$endif$
...
Patienrt: $if(patient)$\texttt{$patient$}$else$TBD$endif$ & Date of report: $if(date)$\texttt{$date$}$else$TBD$endif$\\
 Sample ID: $if(id)$\texttt{$id$}$else$TBD$endif$  &  Institution of origin: $if(institution)$\texttt{$institution$}$else$TBD$endif$\\
 Pre-diagnosis: $if(pre-diagnosis)$\texttt{$pre-diagnosis$}$else$TBD$endif$  & Controls: $if(controls)$\texttt{$controls$}$else$TBD$endif$\\
	&  \\
	\hline
	\end{tabular*}
...
\begin{center} 
\normalsize \bfseries{\slshape{qPCR amplifaction plots}}
\end{center}
